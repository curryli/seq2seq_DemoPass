...
#报错TypeError: can't pickle _thread.lock objects  加下面三句，禁用deepcopy
...
tf.app.flags.DEFINE_integer("numEpochs", 10, "Batch size to use during training.")
...
tf.app.flags.DEFINE_string("train_dir", './model_path', "How many training steps to do per checkpoint.")
...
def create_model(session, forward_only, beam_search, beam_size = 5):
    ...
    model_path = r'model_path/chat_bot.ckpt-0'
    if forward_only:
...
def train():
    # prepare dataset
    data_path = r'data/dataset-cornell-length10-filter1-vocabSize40000.pkl'
